defmodule EventSourcingExampleTest.Events do
  ...
  test "Should create accounts" do
    ...
    result = Bus.forward_event([
      %NewAccount{email: "foo@email.com", password: hash_password, account_number: "1", verify_code: "foo"},
      %NewAccount{email: "bar@email.com", password: hash_password, account_number: "2", verify_code: "foo"},
      %NewAccount{email: "baz@email.com", password: hash_password, account_number: "3", verify_code: "foo"},
    ], [:do_not_log, :do_not_send_email])

    all_accounts = Amnesia.transaction(&Database.Account.all/0)
    all_verify_code = Amnesia.transaction do
  ...
  test "Should fail when creating account using same e-mail" do
    result = Bus.forward_event([
      %NewAccount{email: "foo@email.com", password: "abc123", account_number: "4", verify_code: "foo"}
    ], [:do_not_log, :do_not_send_email])

    assert {:error, [{"already exists an account using this e-mail and/or account number", _}]} = result
  end

  test "Should fail when creating account using same account number" do
    result = Bus.forward_event([
      %NewAccount{email: "qux@email.com", password: "abc123", account_number: "3", verify_code: "foo"}
    ], [:do_not_log, :do_not_send_email])
  ...
  test "Should can transfer money" do
    ...
    {account_1, account_2} = Amnesia.transaction do
      {
        Database.Account.get_account!(%{account_number: "1"}),
